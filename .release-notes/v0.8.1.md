# Release Notes v0.8.1 - Monitoring & Automatisation

**Date de release:** 22 janvier 2026
**Type:** Minor Release
**Score qualité:** 8.5/10 (maintenu)

---

## 🎯 Vue d'ensemble

Cette release se concentre sur l'observabilité, la fiabilité et la qualité avec l'intégration de Sentry, l'automatisation complète des backups, et l'amélioration des tests E2E critiques.

**Résumé des améliorations:**
- ✅ Monitoring erreurs professionnel avec Sentry
- ✅ Backups automatiques multi-plateforme
- ✅ Tests E2E critiques renforcés
- ✅ 35,000+ mots de nouvelle documentation

---

## 🆕 Nouvelles fonctionnalités

### 1. 🔍 Intégration Sentry - Error Tracking & Performance Monitoring

**Backend (Node.js/Express):**
- Capture automatique de toutes les erreurs non gérées
- Performance monitoring avec échantillonnage configurable
- Profiling CPU/mémoire pour insights production
- Contexte de requête (URL, méthode, user agent, user ID)
- Filtrage automatique des données sensibles (Authorization, Cookie)
- Envoi uniquement des erreurs serveur (500+) à Sentry
- Configuration via environnement (`SENTRY_DSN`, `SENTRY_ENVIRONMENT`)

**Frontend (React/TypeScript):**
- Error boundaries React automatiques
- Performance monitoring avec Web Vitals (LCP, FID, CLS)
- Session replay pour debugging (texte masqué pour confidentialité)
- Breadcrumbs des actions utilisateur (clicks, navigation, API calls)
- Intégration React Router pour tracking navigation
- Contexte utilisateur automatique après login/logout
- Filtrage données sensibles (tokens, local storage)

**Packages installés:**
- `@sentry/node@10.36.0`
- `@sentry/profiling-node@8.41.0`
- `@sentry/react@10.36.0`

**Configuration:**
```env
# Backend
SENTRY_DSN=https://...@o....ingest.sentry.io/...
SENTRY_ENVIRONMENT=production
SENTRY_TRACES_SAMPLE_RATE=0.2

# Frontend
VITE_SENTRY_DSN=https://...@o....ingest.sentry.io/...
VITE_SENTRY_TRACES_SAMPLE_RATE=0.3
VITE_SENTRY_REPLAYS_SESSION_SAMPLE_RATE=0.1
VITE_SENTRY_REPLAYS_ON_ERROR_SAMPLE_RATE=1.0
```

**Documentation:**
- Guide complet: `docs/SENTRY_INTEGRATION.md` (8,000+ mots)
- Quick start: `docs/SENTRY_QUICKSTART.md` (3,000+ mots)

---

### 2. 🔄 Automatisation Complète des Backups Database

**Système multi-plateforme:**
- Script Node.js cross-platform: `scripts/backup-automation.js`
- Support Windows (Task Scheduler), Linux/Mac (cron), Docker (crond)
- Scheduling automatique (2h00 AM par défaut, configurable)
- Compression PostgreSQL niveau 9 (format custom)
- Vérification de taille pour détecter les échecs
- Logs détaillés dans `backups/logs/`

**Rétention intelligente:**
- Politique configurable (30 jours par défaut)
- Suppression automatique des backups obsolètes
- Conservation des backups manuels (non supprimés)
- Préservation des backups `pre_restore_*`

**Monitoring HTTP:**
- Service de monitoring: `scripts/backup-monitor.js`
- Endpoint `/health` - Status 200 (healthy) ou 503 (unhealthy)
- Endpoint `/metrics` - Métriques Prometheus-compatible
- Endpoint `/status` - Dashboard HTML avec détails
- Détection backups manquants ou obsolètes

**Installation rapide:**
```bash
# Windows (Administrateur)
scripts\setup-backup-automation.bat

# Linux/Mac
./scripts/setup-backup-automation.sh

# Docker
docker-compose -f docker-compose.yml -f docker-compose.backup.yml up -d
```

**Utilisation:**
```bash
# Backup manuel
node scripts/backup-automation.js

# Backup avec nom personnalisé
node scripts/backup-automation.js --name="avant_upgrade"

# Monitoring
node scripts/backup-monitor.js
# Ouvrir http://localhost:8080/status
```

**Configuration:**
```env
BACKUP_RETENTION_DAYS=30
BACKUP_NOTIFICATION_EMAIL=admin@example.com
DB_CONTAINER=inventaire_si-db-1
BACKUP_MAX_AGE_HOURS=26
HEALTHCHECK_PORT=8080
```

**Documentation:**
- Guide complet: `docs/BACKUP_AUTOMATION.md` (12,000+ mots)
- `backups/README.md` - Mis à jour avec nouvelles fonctionnalités

---

### 3. 🧪 Tests E2E Critiques Renforcés

**Nouveaux tests:**
- `00-smoke.spec.ts` - 10 tests smoke (chemins critiques, ~2 min)
- `11-critical-loan-workflow.spec.ts` - Workflow complet de prêt (~3 min)
- `fixtures.ts` - Factories de données de test réutilisables

**Smoke Tests couvrent:**
- ✅ Login et accès dashboard
- ✅ Navigation menu complète
- ✅ Création employé, modèle d'actif, item
- ✅ Création prêt
- ✅ Export de données
- ✅ Logout et protection routes
- ✅ Recherche fonctionnelle
- ✅ Gestion erreurs

**Critical Loan Workflow teste:**
- ✅ Cycle complet: créer → ajouter items → signature retrait → signature retour → fermer
- ✅ Utilise fixtures pour données de test
- ✅ Cleanup automatique après chaque test

**Test Fixtures disponibles:**
- `createTestEmployee()` - Créer employé avec données uniques
- `createTestAssetModel()` - Créer modèle d'actif
- `createTestAssetItem()` - Créer item d'actif
- `createTestStockItem()` - Créer item de stock
- `createTestLoan()` - Créer prêt
- `cleanupTestData()` - Nettoyage complet

**CI/CD GitHub Actions:**
- Workflow complet: `.github/workflows/e2e-tests.yml`
- Déclencheurs: PRs, pushes main, nightly, manuel
- Services: PostgreSQL 16 + Redis 7
- Exécution: smoke tests (fail fast) → critical tests → all tests
- Upload: rapports HTML + vidéos échecs + traces
- Commentaires automatiques résultats sur PRs

**Helper Scripts:**
```bash
# Windows
scripts\run-e2e-tests.bat smoke        # Smoke tests
scripts\run-e2e-tests.bat critical     # Tests critiques
scripts\run-e2e-tests.bat              # Tous les tests

# Linux/Mac
./scripts/run-e2e-tests.sh smoke
./scripts/run-e2e-tests.sh critical
./scripts/run-e2e-tests.sh
```

**Documentation:**
- Guide complet: `docs/E2E_TESTING.md` (11,000+ mots)

---

## 📚 Documentation

**Nouveaux documents (35,000+ mots):**
- `docs/SENTRY_INTEGRATION.md` - Guide complet Sentry (8,000 mots)
- `docs/SENTRY_QUICKSTART.md` - Quick start Sentry (3,000 mots)
- `docs/BACKUP_AUTOMATION.md` - Guide backups automatiques (12,000 mots)
- `docs/E2E_TESTING.md` - Guide tests E2E Playwright (11,000 mots)
- `COURT_TERME_COMPLETE.md` - Résumé complet des améliorations

**Fichiers mis à jour:**
- `README.md` - Ajout section v0.8.1
- `backups/README.md` - Nouvelles fonctionnalités
- `apps/web/package.json` - Package Sentry

---

## 🔧 Fichiers ajoutés

**Backend Sentry:**
- `apps/api/src/config/sentry.js` - Configuration Sentry Node.js
- Modifications: `apps/api/src/index.js`, `apps/api/src/app.js`, `apps/api/src/middleware/errorHandler.js`
- `.env.example` - Variables Sentry

**Frontend Sentry:**
- `apps/web/src/lib/sentry.ts` - Configuration Sentry React
- Modifications: `apps/web/src/main.tsx`, `apps/web/src/App.tsx`, `apps/web/src/lib/hooks/useAuth.ts`
- `.env.example` - Variables Sentry

**Backups:**
- `scripts/backup-automation.js` - Script principal Node.js
- `scripts/backup-monitor.js` - Service monitoring HTTP
- `scripts/setup-backup-automation.bat` - Installation Windows
- `scripts/setup-backup-automation.sh` - Installation Linux/Mac
- `docker-compose.backup.yml` - Configuration Docker

**Tests E2E:**
- `apps/web/e2e/00-smoke.spec.ts` - Tests smoke
- `apps/web/e2e/11-critical-loan-workflow.spec.ts` - Workflow prêt
- `apps/web/e2e/fixtures.ts` - Factories de données
- `.github/workflows/e2e-tests.yml` - CI/CD workflow
- `scripts/run-e2e-tests.bat` - Helper Windows
- `scripts/run-e2e-tests.sh` - Helper Linux/Mac

**Total:** 22 nouveaux fichiers

---

## 🎯 Métriques

**Avant v0.8.1:**
- Tests: 292 tests (backend + frontend existants)
- Documentation: ~50,000 mots
- Monitoring: Aucun
- Backups: Manuel uniquement (scripts PowerShell existants)
- Tests E2E: Existants mais test prêt désactivé

**Après v0.8.1:**
- Tests: 300+ tests (ajout smoke + critical workflow)
- Documentation: ~85,000 mots (+35,000 mots)
- Monitoring: Sentry intégré (backend + frontend)
- Backups: Automatiques multi-plateforme avec monitoring
- Tests E2E: Chemins critiques fonctionnels + CI/CD

**Impact:**
- 🎯 Détection erreurs production en temps réel
- 📊 Métriques performance (API + Web Vitals)
- 🤖 Backups quotidiens garantis sans intervention
- 📈 Monitoring santé backups avec alertes
- ✅ Tests critiques automatisés sur PRs

---

## 🚀 Installation & Configuration

### Activer Sentry

1. Créer projets Sentry (backend + frontend): https://sentry.io/
2. Copier les DSN
3. Configurer `.env`:
   ```env
   # Backend
   SENTRY_DSN=https://...@o....ingest.sentry.io/...

   # Frontend
   VITE_SENTRY_DSN=https://...@o....ingest.sentry.io/...
   ```
4. Redémarrer serveurs
5. Les erreurs sont automatiquement envoyées!

### Activer Backups Automatiques

**Windows:**
```batch
# Ouvrir PowerShell en Administrateur
scripts\setup-backup-automation.bat
```

**Linux/Mac:**
```bash
chmod +x scripts/setup-backup-automation.sh
./scripts/setup-backup-automation.sh
```

**Docker:**
```bash
docker-compose -f docker-compose.yml -f docker-compose.backup.yml up -d
```

### Tester E2E

```bash
# Smoke tests (rapide)
npm run test:e2e -- 00-smoke.spec.ts

# Tests critiques
scripts/run-e2e-tests.bat critical

# Mode UI interactif
npm run test:e2e:ui
```

---

## 📊 Compatibilité

**Node.js:** >= 20.x (inchangé)
**Docker:** >= 20.x (inchangé)
**PostgreSQL:** 16 (inchangé)
**Redis:** 7 (inchangé)
**Navigateurs (E2E):** Chromium (Playwright)

**Nouveaux packages:**
- Backend: `@sentry/node@10.36.0`, `@sentry/profiling-node@8.41.0`
- Frontend: `@sentry/react@10.36.0`

---

## 🔄 Migration depuis v0.8.0

**Pas de breaking changes.** Tous les changements sont additifs.

**Étapes optionnelles:**

1. **Activer Sentry** (optionnel mais recommandé):
   - Créer projets Sentry
   - Configurer DSN dans `.env`
   - Redémarrer serveurs

2. **Activer backups automatiques** (optionnel mais recommandé):
   - Exécuter script d'installation approprié
   - Vérifier premier backup
   - Configurer monitoring (optionnel)

3. **Mettre à jour dépendances frontend:**
   ```bash
   cd apps/web
   npm install --legacy-peer-deps
   ```

4. **Tester les nouveaux tests E2E:**
   ```bash
   npm run test:e2e -- 00-smoke.spec.ts
   ```

---

## 🐛 Bugs corrigés

### Production Docker — Fixes critiques (28 janvier 2026)

- **Fix CORS production** : les requêtes sans header `Origin` (healthchecks, monitoring) étaient bloquées en mode production
- **Fix Sentry v8** : `ProfilingIntegration` n'est plus exporté → import dynamique conditionnel ; `Sentry.Handlers` supprimé en v8 → wrapping conditionnel
- **Fix CSRF middleware** : import `UnauthorizedError` depuis le mauvais module ; chemins `EXEMPT_ROUTES` ne matchaient pas (préfixe `/api` strippé par Express)
- **Fix rate limiter** : `generalLimiter` à 100 req/15min bloquait le frontend en parallèle → porté à 500
- **Fix healthcheck web Docker** : `wget http://localhost/` échoue dans l'image nginx alpine → changé à `127.0.0.1`
- **Fix clignotement login/dashboard** : après rechargement de page, le store Zustand rehydrait `isAuthenticated=true` sans token en mémoire → le dashboard se chargait puis redirectait vers login. Ajout d'une tentative de refresh token dans `ProtectedRoute` avant le premier rendu

---

## ⚠️ Notes importantes

1. **Sentry nécessite configuration:**
   - DSN backend et frontend différents
   - Variables d'environnement requises
   - Voir `docs/SENTRY_QUICKSTART.md`

2. **Backups automatiques:**
   - Nécessite Node.js 20+ et Docker
   - Windows: droits Administrateur pour Task Scheduler
   - Linux/Mac: cron doit être actif

3. **Tests E2E:**
   - CI/CD nécessite GitHub Actions activé
   - Tests locaux nécessitent Docker + DB running
   - Playwright Chromium installé automatiquement

4. **Compatibilité React 19:**
   - Package Sentry installé avec `--legacy-peer-deps`
   - Aucun impact sur fonctionnalité
   - Sera résolu dans futures versions Sentry

---

## 🔜 Prochaines étapes recommandées

### Court terme (1-2 semaines)

1. **Activer Sentry en production**
   - Configurer DSN
   - Tester avec erreurs de test
   - Configurer alertes

2. **Activer backups automatiques**
   - Installer scheduling
   - Vérifier premiers backups
   - Tester restauration

3. **Intégrer E2E dans workflow**
   - Activer GitHub Actions
   - Exécuter smoke tests avant commits
   - Corriger tests flaky

### Moyen terme (1 mois)

1. **Monitoring avancé:**
   - Intégrer Prometheus/Grafana
   - Créer dashboards personnalisés
   - Configurer alertes critiques

2. **Backups off-site:**
   - Configurer copie cloud (S3, Azure)
   - Tester disaster recovery
   - Documenter procédure

3. **Extension tests:**
   - Ajouter tests features manquantes
   - Tests performance (Lighthouse)
   - Tests accessibilité (axe-core)

---

## 👥 Contributeurs

- **Mickael GERARD** - Développement principal
- **Claude Sonnet 4.5** - Assistant développement

---

## 📝 Changelog complet

Voir [CHANGELOG.md](../CHANGELOG.md) pour l'historique détaillé.

---

**Release v0.8.1 - Production-Ready avec Monitoring Professionnel 🚀**

*Date: 22 janvier 2026*
